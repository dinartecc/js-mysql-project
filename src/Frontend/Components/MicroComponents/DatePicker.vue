    ...
    <div>
      ...
      <div class='calender-div' :class='{noDisplay: hideCal}'>
        <div :class='{noDisplay: hideDate}'>
          ...
          <div>
            <div class="week" v-for="(week, weekIndex) in weeks" :key="week">
              <span class="port" v-for="(day, dayIndex) in week" :key="day" v-on:click='setDay(weekIndex*7 + dayIndex, day)' :class='{activePort: (weekIndex*7 + dayIndex) === activePort}'>
                {{day}}
              </span>
            </div>
          </div>
